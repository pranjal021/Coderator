export default {
  ...
  template: `
    <div id="file-explorer" class="text-white py-2">
      <medium class="px-2">File Explorer</medium>
      <i class="las la-file-medical float-end me-2" title="Create new file" @click="newFile"></i>
        ...
        <ul class="list-group list-group-flush">
          <li class="list-group-item" v-for="(file,index) in files" :key="index" :class="{active: index==active}" @click="openFile(index)">
            <small class="d-flex align-items-center">
              ...
              <i class="las la-times float-end" title="Close file" @click="removeFile(index)"></i>
            </small>
          </li>
          ...
          <li class="list-group-item" v-show="flag">
            <small>
              <div class="input-group mb-3">
                ...
                <input maxlength="100" placeholder="Enter file name" type="text" class="form-control py-0 border-0 bg-transparent" ref="filename" @blur="addFile"
  ...
  methods: {
    ...
    removeFile(index) {
      if (this.files.length > 1) {
        ...
        alert("Cannot delete all the files");
      }
    },
